...
import { SKILLS_DIR } from "../constants/gemini.js";
...
/**
 * Returns the path to a skill's SKILL.md in the Gemini target directory.
 */
export function geminiSkillPath(name: string): string {
...
async function targetSkillExists(name: string): Promise<boolean> {
	try {
		await readFile(geminiSkillPath(name), "utf8");
		return true;
	} catch {
		return false;
	}
...
export class GeminiSkillAdapter implements ProviderAdapter<string> {
	/**
	 * Read the current content of `~/.gemini/skills/<skillName>/SKILL.md`.
	 * Returns an empty string when the file does not exist.
	 */
	async readTarget(skillName?: string): Promise<string> {
		if (!skillName) return "";
		try {
			return await readFile(geminiSkillPath(skillName), "utf8");
		} catch {
			return "";
		}
	}
	 ...
	 * 2. Copy each to `~/.gemini/skills/<name>/SKILL.md` (pure file copy).
	...
	async sync(_source: OpenCodeConfig): Promise<void> {
		...
		for (const { name, content } of skills) {
			...
			if (exists && !manifest.includes(name)) {
				console.warn(
					`[relay:skill] Skill "${name}" already exists in ~/.gemini/skills/ and was not placed by the relay — overwriting with OpenCode version`,
				);
			}
